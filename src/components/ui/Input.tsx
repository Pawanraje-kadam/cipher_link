    ...
    return (
      <div className="w-full">
        {label && (
          <label
            htmlFor={inputId}
            className="block text-[15px] font-mono font-bold uppercase tracking-[0.14em] text-bone-200 mb-2"
          >
            {label}
          </label>
        )}
        <input
          ...
          {...props}
        />
        {hint && <p className="mt-1.5 text-[15px] font-mono font-bold text-bone-300">{hint}</p>}
      </div>
    );
  },
);
